...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		pr_err("%s: Audio notifier register failed ret = %d\n",
			__func__, ret);
	#ifdef OPLUS_ARCH_EXTENDS
	pr_info("%s: set de-activated %s", __func__, "dmic_gpio");
	if (dmic1_highz_enable(1)) {
		pr_err("%s: cannot de-activated %s", __func__, "dmic_gpio");
	}
	#if defined(CONFIG_SND_SOC_IA6XX)
	...
	#ifdef OPLUS_ARCH_EXTENDS
	pr_debug("%s: sound card register success.\n", __func__);
	#endif /* OPLUS_ARCH_EXTENDS */
	return 0;
err:
