					     ...
					     bool active)
{
	...
	if (active) {
		...
		if (mbhc->jack_state == OPPO_USBC_JACK_INVALID || mbhc->jack_state == OPPO_USBC_JACK_SELFSTICK) {
		...
		#ifdef OPLUS_ARCH_EXTENDS
		if (mbhc->jack_state == OPPO_USBC_JACK_INVALID || mbhc->jack_state == OPPO_USBC_JACK_SELFSTICK) {
			mbhc->jack_state = OPPO_USBC_JACK_NONE;
...
static void wcd_mbhc_usbc_abnormal_adapter_dwork_fn(struct work_struct *work)
{
	...
	if (OPPO_USBC_JACK_SELFSTICK == mbhc->jack_state) {
		extcon_set_state_sync(&mbhc->selfstick_dev, EXTCON_JACK_SELFSTICK_BTN_PLUG_IN, true);
		mbhc->invalid_jack_cound = 0;
	}
	else if (OPPO_USBC_JACK_INVALID == mbhc->jack_state && mbhc->invalid_jack_cound < FSA4480_CHECK_INVALID_JACK_COUND_MAX) {
