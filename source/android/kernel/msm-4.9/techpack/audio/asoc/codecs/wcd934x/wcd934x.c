...
//Add Qcom CR#1110511, solve tx overflow/underflow
...
#ifdef OPLUS_ARCH_EXTENDS
static int micbias4_ctl_status;
static int micbias4_ctl_switch_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
    pr_info("%s micbias4_ctl_status:%d \n", __func__,micbias4_ctl_status);
	ucontrol->value.integer.value[0] = micbias4_ctl_status;
	return 0;
}

static int micbias4_ctl_switch_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);

	pr_info("%s  micbias set %ld : \n",__func__, ucontrol->value.integer.value[0]);

	dev_err(codec->dev, "%s enter \n", __func__);
	dev_err(codec->dev, "%s  micbias set %ld : \n",__func__, ucontrol->value.integer.value[0]);
	if(ucontrol->value.integer.value[0]){
	    tavil_cdc_mclk_enable(codec, true);
		tavil_micbias_control(codec, MIC_BIAS_4, MICB_ENABLE, false);
	} else {
	    tavil_micbias_control(codec, MIC_BIAS_4, MICB_DISABLE, false);
		tavil_cdc_mclk_enable(codec, false);
	}
	micbias4_ctl_status = ucontrol->value.integer.value[0];
...
#ifdef OPLUS_ARCH_EXTENDS
static char const *micbias4_ctl_switch_text[] = {"Off", "On"};
static SOC_ENUM_SINGLE_EXT_DECL(micbias4_ctl_switch_enum, micbias4_ctl_switch_text);
#endif /* OPLUS_ARCH_EXTENDS */
...
static const struct snd_kcontrol_new tavil_snd_controls[] = {
	...
	#ifdef OPLUS_ARCH_EXTENDS
	SOC_ENUM_EXT("MICBIAS4 CTL Switch", micbias4_ctl_switch_enum,
		micbias4_ctl_switch_get, micbias4_ctl_switch_set),
	#endif /* OPLUS_ARCH_EXTENDS */
