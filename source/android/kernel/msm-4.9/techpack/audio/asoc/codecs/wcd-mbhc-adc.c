...
#define pr_debug pr_info
	...
	if (hphl_adc_res > 100 && hphr_adc_res > 100) {
		...
		#ifdef OPLUS_ARCH_EXTENDS
		if ((mbhc->jack_state == OPPO_USBC_JACK_GND_MIC_SWAP) &&
				(hphl_adc_res < 500 && hphr_adc_res < 500)) {
			mbhc->jack_state = OPPO_USBC_JACK_INVALID;
			pr_debug("%s: switch may not work or it's selfstick!!!!\n", __func__);
		}
		#endif /* OPLUS_ARCH_EXTENDS */
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	int plug_type_default = MBHC_PLUG_TYPE_INVALID;
	#endif /* OPLUS_ARCH_EXTENDS */
...
#define pr_debug pr_info
	...
	#else /* OPLUS_ARCH_EXTENDS */
	/* Check for cross connection */
	if (is_project(OPPO_17107) && (HW_VERSION__19 == get_PCB_Version()) && mbhc->jack_state == OPPO_USBC_JACK_NONE){
		msleep(150); //form plug in to hear need 150ms, add another 150ms to 300ms
		output_mv = wcd_measure_adc_continuous(mbhc);
		plug_type_default = wcd_mbhc_get_plug_from_adc(mbhc, output_mv);
		do {
			cross_conn = wcd_check_cross_conn(mbhc);
			try++;
		} while (try < mbhc->swap_thr);
		...
		if (cross_conn > 0) {
			plug_type = MBHC_PLUG_TYPE_GND_MIC_SWAP;
			mbhc->jack_state = OPPO_USBC_JACK_GND_MIC_SWAP;
			pr_debug("%s: cross connection found, Plug type %d\n",
				__func__, plug_type);
			goto correct_plug_type;
			...
			if (wcd_check_cross_conn(mbhc) && (!wcd_swch_level_remove(mbhc))){
				as6313_CTIA();
				mbhc->jack_state = OPPO_USBC_JACK_NO_GND_MIC_SWAP;
				pr_debug("%s: lile add, switch CTIA, it's headset or HIGH_HPH\n", __func__);
			} else {
				mbhc->jack_state = OPPO_USBC_JACK_HPL_HPR_GND;
	...
	if (is_project(OPPO_17107) && (HW_VERSION__19 == get_PCB_Version()) &&
		...
		if (plug_type == MBHC_PLUG_TYPE_HEADPHONE && (!wcd_swch_level_remove(mbhc))) {
			...
			as6313_CTIA();
			plug_type = plug_type_default;
			if (plug_type_default == MBHC_PLUG_TYPE_HIGH_HPH) {
				mbhc->jack_state = OPPO_USBC_JACK_TIELINE;
				pr_err("%s: after recheck, switch CTIA it's tieline\n", __func__);
			} else {
				mbhc->jack_state = OPPO_USBC_JACK_HEADPHONES;
				pr_err("%s: after recheck, switch CTIA, it's headphones\n", __func__);
			}
		} else if (plug_type == MBHC_PLUG_TYPE_HIGH_HPH){
			mbhc->jack_state = OPPO_USBC_JACK_TIELINE;
			pr_err("%s: after recheck, switch OMTP, it's tieline\n", __func__);
		} else if ((plug_type == MBHC_PLUG_TYPE_HEADSET) && (plug_type_default == MBHC_PLUG_TYPE_HEADSET)){
			mbhc->jack_state = OPPO_USBC_JACK_SELFSTICK;
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		#else
		if (is_project(OPPO_17107) && (HW_VERSION__19 == get_PCB_Version()) &&
				((mbhc->jack_state == OPPO_USBC_JACK_GND_MIC_SWAP) || (mbhc->jack_state == OPPO_USBC_JACK_INVALID)) &&
				(output_mv <= hs_threshold) &&(!is_pa_on)){
			/* Check for cross connection*/
			ret = wcd_check_cross_conn(mbhc);
			...
			if (ret < 0)
				...
				if (pt_gnd_mic_swap_cnt <
					...
					*/
					pr_debug("%s: switch did not work\n",
						__func__);
					if (mbhc->jack_state == OPPO_USBC_JACK_INVALID) {
						pr_debug("%s: for AS6313, it maybe selfstick\n",
						 __func__);
						mbhc->jack_state = OPPO_USBC_JACK_SELFSTICK;
						plug_type = MBHC_PLUG_TYPE_HEADSET;
					} else {
...
exit:
	...
	#ifdef OPLUS_ARCH_EXTENDS
	if (is_project(OPPO_17107) && (HW_VERSION__19 == get_PCB_Version()) && (mbhc->jack_state == OPPO_USBC_JACK_SELFSTICK)){
...
static irqreturn_t wcd_mbhc_adc_hs_rem_irq(int irq, void *data)
{
	...
	/* use fsa4480 to recheck if be plug out */
	if (!is_project(OPPO_17107) || (HW_VERSION__19 != get_PCB_Version())){
